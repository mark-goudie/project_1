...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let chainError = await self.validateChain();
        if (chainError.length > 0) {
          reject("Chain is invalid, block has not been added.");
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (currentTime - messageTime > timeLimit) {
          reject("Validation period exceeded");
          return;
        }
  ...
  getBlockByHash(hash) {
    ...
    return new Promise((resolve, reject) => {
      const findBlock = this.chain.filter((block) => block.hash === hash);
      resolve(block);
    });
  }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (let i = 1; i < self.chain.length; i++) {
        ...
        try {
          ...
          if (!isValid) {
            errorLog.push("invalid block at " + block.height);
          }
